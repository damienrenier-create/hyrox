...
export async function buildLevelRecords(f: RecordFilters = {}): Promise<RecordsResult> {
  ...
  const boards: RecordBoard[] = phase === "finisher" ? [
    ...
    top("levels", phase === "warmup" ? "🔥 Le plus de séries bouclées" : "🧗 Le niveau le plus haut", `${unit === "niv." ? "niveaux" : "séries"} bouclé(e)s, puis le moins de vies perdues, puis le plus rapide`, (r) => (r.levels > 0 || r.currentDone > 0 ? r.levels : null), false, (v, r) => `${r.finishMs !== null ? `🏁 ${v} en ${fmt(r.finishMs)}` : `${v} ${unit} + ${r.currentDone} fiche${r.currentDone > 1 ? "s" : ""}`}${r.losses ? ` · 💔 ${r.losses}` : ""}`, lvlTie),
    ...
    top("reps", "💪 Le plus de reps", "fiches entières validées", (r) => (r.reps > 0 ? r.reps : null), false, (v) => `${v} reps`),
    top("work", "⚖️ Le plus de travail", "reps × pondération cumulées", (r) => (r.work > 0 ? r.work : null), false, (v) => fmtTheoretical(v)),
